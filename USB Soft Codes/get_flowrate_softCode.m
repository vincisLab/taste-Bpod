function flow = get_flowrate_softCode(MFCnum)
    ...
    send_command(o, cmd_get);%first, flush the buffer on the teensy
    confirmation = send_command(o, cmd);
    while(isempty(confirmation) || ~startsWith(confirmation, 'MFC set'))
        confirmation = send_command(o, cmd);
    end
    returnStr = send_command(o, cmd_get);
