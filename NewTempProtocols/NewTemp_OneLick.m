...
%     S.A.startLogging()                                                    ;                            % starts logging samples from channels 1 to nActiveChannels
    ...
    switch currentTrial(1,1)
        ...
        case 8 % spout in the rig: number 4
...
% 5th state - delay 4 sec to allow aspiration of all liquid in excess.
...
sma = AddState(sma, 'Name', 'BNC1', ...           % send a time stamp to analog 2 Plexon (use to realigned all the data  )
    'Timer', 0.001,...
    'StateChangeConditions', {'Tup', 'LateralMotor'},...
    'OutputActions', {'BNCState', 1 }); % 

sma = AddState(sma, 'Name', 'LateralMotor', ...   % move the lateral motor to put in position the correct spout
    'Timer', 2,...
...
sma = AddState(sma, 'Name', 'WaitForLick1_IN', ...
        ...
        'StateChangeConditions', {'Port2In', 'WaitForLick1_OUT','Tup','ZaberB1'},... % to change it to ZaberB1 Rinse if we will add the rinse
        'OutputActions', {}); 
%     
...
%        flagPlotRinse  = any(~isnan(BpodSystem.Data.RawEvents.Trial{1,T}.States.OpenValveRinse));
         flagPlotRinse  = any(~isnan(BpodSystem.Data.RawEvents.Trial{1,T}.States.controlstate)); % change it to controlstate2 as soon as we add the rinse
